...
class PngPage {
    ...
    async render(imageIndex) {
        let blob = await this.offscreenCanvas.convertToBlob();
        const img = $("img", this.outputDiv)[imageIndex];
...
function drawRound(amqRound, foo) {
    ...
    if (filterStr) {
        ...
        const filters = {
            ...
            composer: (song) => song.songInfo.composerInfo?.name.toLowerCase().includes(filterStr) || false,
            arranger: (song) => song.songInfo.arrangerInfo?.name.toLowerCase().includes(filterStr) || false,
